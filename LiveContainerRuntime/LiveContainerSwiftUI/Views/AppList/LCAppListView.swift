...
struct LCAppListView : View, LCAppBannerDelegate, LCAppModelDelegate {
    ...
    @State private var isCheckingEmberInbox = false
    ...
    /// Imports IPAs delivered by Ember Connect Desktop over the app's
    /// file-sharing Documents container. Each IPA goes through LiveContainer's
    /// real patch/sign/install pipeline; this is deliberately not a raw unzip.
    func installFromEmberInbox() async {
        guard !isCheckingEmberInbox, !installprogressVisible, !errorShow else { return }
        isCheckingEmberInbox = true
        defer { isCheckingEmberInbox = false }
        ...
        // Ember Desktop exports the same development certificate that signed
        // this host. Import it automatically so JIT-less guests work without
        // sending the user through LiveContainer's advanced settings screens.
        importEmberCertificateIfPresent(in: documentsURL)
        guard !errorShow else { return }

        let inboxURL = documentsURL.appendingPathComponent("AutoInstall", isDirectory: true)
        do {
            try fileManager.createDirectory(at: inboxURL, withIntermediateDirectories: true)
            let ipaURLs = try fileManager.contentsOfDirectory(
                at: inboxURL,
            ...
            errorInfo = "Ember Connect could not read its incoming app folder: \(error.localizedDescription)"
            errorShow = true
        }
    }

    private func importEmberCertificateIfPresent(in documentsURL: URL) {
        let fileManager = FileManager.default
        let candidates = ["certificate.p12", "livecontainer.p12"]
            .map { documentsURL.appendingPathComponent($0) }
        guard let certificateURL = candidates.first(where: { fileManager.fileExists(atPath: $0.path) }),
              let certificateData = try? Data(contentsOf: certificateURL) else {
            return
        }
        let passwordURL = documentsURL.appendingPathComponent("certificate-password.txt")
        if let existingCertificate = LCUtils.certificateData(),
           existingCertificate == certificateData {
            for candidate in candidates { try? fileManager.removeItem(at: candidate) }
            try? fileManager.removeItem(at: passwordURL)
            return
        }

        let exportedPassword = (try? String(contentsOf: passwordURL, encoding: .utf8))?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let password = (exportedPassword?.isEmpty == false) ? exportedPassword! : "password"

        guard LCUtils.getCertTeamId(withKeyData: certificateData, password: password) != nil else {
            errorInfo = "The signing certificate sent by Ember Connect Desktop is invalid or has the wrong password. Export it again from the desktop app."
            errorShow = true
            return
        }
        ...
        // The imported values now live in the app's defaults container. Do not
        // leave a password and its certificate exposed through iOS File
        // Sharing longer than necessary.
        for candidate in candidates { try? fileManager.removeItem(at: candidate) }
        try? fileManager.removeItem(at: passwordURL)
    }
