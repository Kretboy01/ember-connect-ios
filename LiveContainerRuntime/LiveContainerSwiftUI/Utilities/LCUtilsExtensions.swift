        ...
        if(jitEnabler == .SideJITServer){
            ...
            if jitEnabler == .StosDebugLC {
                ...
                guard let appToLaunch else {
                    onServerMessage?("StosDebug is not installed in Ember Connect.")
                    return false
                }
                
                if !appToLaunch.uiIsShared {
                    onServerMessage?("StosDebug is installed in Ember Connect, but is not a shared app. Convert it to a shared app to continue.")
                    return false
                }
                ...
                guard let freeScheme else {
                    onServerMessage?("No free Ember Connect is available. Please either: \n(1)close one, \n(2)install a new one, \n(3)choose another method to enable JIT.")
                    return false
                }
                ...
                onServerMessage?("JIT acquisition will continue in another Ember Connect.")
            ...
            if jitEnabler == .StikJITLC {
                ...
                guard let appToLaunch else {
                    onServerMessage?("StikDebug is not installed in Ember Connect.")
                    return false
                }
                
                if !appToLaunch.uiIsShared {
                    onServerMessage?("StikDebug is installed in Ember Connect, but is not a shared app. Convert it to a shared app to continue.")
                    return false
                }
