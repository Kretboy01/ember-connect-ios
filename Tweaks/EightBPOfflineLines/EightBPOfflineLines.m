...
static NSString *const ECTableOverlayKey = @"EmberEightBPOfflineLines.tableOverlay";
static NSString *const ECAutoplayKey = @"EmberEightBPOfflineLines.autoplay";
...
static BOOL gECAutoplayEnabled = NO;
static BOOL gECAutoplayShotPending = NO;
...
static CFTimeInterval gECAutoplayBallPlacementAt = 0;
static BOOL gECAutoplayBallPlacementFinalized = NO;
...
static void ECGameManagerOnExit(id self, SEL selector) {
    ...
    gECAutoplayBallPlacementAt = 0;
    gECAutoplayBallPlacementFinalized = NO;
...
static void ECClearTableOverlay(void) {
    if (!gECTableOverlaySprites) return;
    for (id sprite in gECTableOverlaySprites.copy) {
        if ([sprite respondsToSelector:@selector(removeFromParent)]) {
            ((void (*)(id, SEL))objc_msgSend)(sprite, @selector(removeFromParent));
        }
    }
    [gECTableOverlaySprites removeAllObjects];
}

static void ECAddTableOverlayLine(id parent, ECDPoint from, ECDPoint to,
                                  ECccColor3B color, id referenceBall, long long z,
                                  double thickness) {
    ...
    double dy = end.y - start.y;
    double length = hypot(dx, dy);
    if (!isfinite(length) || length < 0.4) return;
    id texture = ECSolidLineTexture();
    Class spriteClass = NSClassFromString(@"CCSprite");
    if (!texture || !spriteClass) return;
    id sprite = ((id (*)(id, SEL))objc_msgSend)(spriteClass, @selector(alloc));
    ...
    CGSize size = [sprite respondsToSelector:@selector(contentSize)]
        ? ((CGSize (*)(id, SEL))objc_msgSend)(sprite, @selector(contentSize)) : CGSizeMake(1, 1);
    if ([sprite respondsToSelector:@selector(setAnchorPoint:)]) {
        ((void (*)(id, SEL, CGPoint))objc_msgSend)(sprite, @selector(setAnchorPoint:),
                                                   CGPointMake(0, 0.5));
    }
    if ([sprite respondsToSelector:@selector(setPosition:)]) {
        ((void (*)(id, SEL, CGPoint))objc_msgSend)(sprite, @selector(setPosition:), start);
    ...
    if ([sprite respondsToSelector:@selector(setScaleY:)]) {
        ((void (*)(id, SEL, float))objc_msgSend)(sprite, @selector(setScaleY:),
                                                 (float)(thickness / MAX(1.0, size.height)));
    }
    if ([sprite respondsToSelector:@selector(setColor:)]) {
        ((void (*)(id, SEL, ECccColor3B))objc_msgSend)(sprite, @selector(setColor:), color);
    }
...
static int ECReadPocketCenters(id table, ECDPoint *pockets, int capacity, double *radiusOut) {
    if (!table || !pockets || capacity <= 0) return 0;
    id properties = ECInvokeId(table, @"tableProperties");
    if (!ECLooksLikeObject(properties)) return 0;
    SEL pocketsSelector = NSSelectorFromString(@"getPockets");
    if (![properties respondsToSelector:pocketsSelector]) return 0;
    const ECPointVector *vector = ((const ECPointVector *(*)(id, SEL))objc_msgSend)(
        properties, pocketsSelector);
    if (!vector || !vector->begin || !vector->end || vector->end < vector->begin) return 0;
    ptrdiff_t count = vector->end - vector->begin;
    if (count < 1 || count > 16) return 0;
    int written = 0;
    for (ptrdiff_t index = 0; index < count && written < capacity; index++) {
        ECDPoint point = vector->begin[index];
        if (!ECPointValid(point)) continue;
        pockets[written++] = point;
    }
    if (radiusOut) {
        double fallback = 6.7;
        id cue = gECCachedCueBall;
        double ballRadius = ECBallLiveRadius(cue);
        if (ballRadius > 0.5) fallback = ballRadius * 1.85;
        *radiusOut = fallback;
    }
    return written;
}
...
static BOOL ECReadIntegerIvar(id object, const char *name, int *valueOut) {
    ...
    if (!ivar) return NO;
    const char *type = ivar_getTypeEncoding(ivar) ?: "";
    if (type[0] != 'i' && type[0] != 'I' && type[0] != 's' && type[0] != 'S') return NO;
    const uint8_t *bytes = (const uint8_t *)(__bridge const void *)object;
    int value = 0;
    if (type[0] == 's' || type[0] == 'S') {
        uint16_t shortValue = 0;
        memcpy(&shortValue, bytes + ivar_getOffset(ivar), sizeof(shortValue));
...
static unsigned int ECAutoplayCurrentShotId(id manager) {
    int value = 0;
    return ECReadIntegerIvar(manager, "mCurrentShotId", &value) && value >= 0
        ? (unsigned int)value : 0;
}

static NSArray *ECAutoplayBalls(id table) {
    id balls = ECInvokeId(table, @"balls");
    if (![balls isKindOfClass:NSArray.class]) balls = ECIvarObject(table, "mBalls");
    return [balls isKindOfClass:NSArray.class] ? balls : nil;
}

static BOOL ECAutoplayBallsMoving(NSArray *balls) {
    for (id ball in balls) {
        if (!ECLooksLikeObject(ball) || ECBallIsPotted(ball)) continue;
        SEL selector = NSSelectorFromString(@"isMovingOrSpinning");
        if ([ball respondsToSelector:selector] &&
            ((BOOL (*)(id, SEL))objc_msgSend)(ball, selector)) return YES;
    }
    return NO;
}
...
static BOOL ECAutoplayBallIsLegal(id manager, id ball, int number) {
    ...
    // Game state 4 uses mPlayer and state 7 uses mOpponent.  The old code
    // always classified against mPlayer, so Pass & Play's second side aimed at
    // the wrong colour.  getCurrentPlayer is the game's own state-aware path.
    id player = ECInvokeId(manager, @"getCurrentPlayer");
    if (!player) {
        BOOL opponentTurn = ECInvokeBool(manager, @"isOpponentTurn", NO);
        player = ECIvarObject(manager, opponentTurn ? "mOpponent" : "mPlayer");
    }
    ...
    if (number == 8) {
        ...
        for (id other in balls) {
            ...
            if (otherNumber > 0 && otherNumber != 8 && !ECBallIsPotted(other)) return NO;
        }
    }
    return YES;
}
...
static BOOL ECAutoplayChoosePlan(id manager, id table, id cueBall, ECAutoplayPlan *planOut) {
    ...
    if (balls.count < 2) return NO;
    ECDPoint cuePosition = ECBallLivePosition(cueBall);
    double cueRadius = ECBallLiveRadius(cueBall);
    if (!ECPointValid(cuePosition) || cueRadius <= 0.0) return NO;
    ...
    for (id ball in balls) {
        ...
        if (!ECAutoplayBallIsLegal(manager, ball, number)) continue;
        ...
        for (int pocketIndex = 0; pocketIndex < pocketCount; pocketIndex++) {
            if (number > 0 && number < 16 && pocketIndex < 8 &&
                (gECAutoplayRejectedPockets[number] & (uint8_t)(1u << pocketIndex))) {
                continue;
            }
            ECDPoint pocket = pockets[pocketIndex];
            ECDPoint pocketVector = {pocket.x - ballPosition.x,
                                     pocket.y - ballPosition.y};
            double objectDistance = hypot(pocketVector.x, pocketVector.y);
            if (!isfinite(objectDistance) || objectDistance < ballRadius * 1.2) continue;
            ECDPoint pocketDirection = {pocketVector.x / objectDistance,
                                        pocketVector.y / objectDistance};
            ECDPoint ghost = {
                ballPosition.x - pocketDirection.x * (cueRadius + ballRadius),
                ballPosition.y - pocketDirection.y * (cueRadius + ballRadius),
            };
            ...
            // A known scratch is never a playable candidate.  The previous
            // score penalty still selected it whenever every available pot
            // scratched, which made autoplay deliberately sink the cue ball.
            ...
            // pointToTablePosition: treats its point as the finite end of the
            // native guide query. The mathematical ghost is exactly where the
            // cue-ball centre first touches the object ball; stopping there
            // left the native query one epsilon short and reported no hit.
            // Extend through that contact without changing the aim direction.
            ECDPoint nativeAimPoint = {
                ghost.x + cueDirection.x * cueRadius * 6.0,
                ghost.y + cueDirection.y * cueRadius * 6.0,
            };
            ...
            if (score < best.score) {
                best = (ECAutoplayPlan){
                    ...
                    .pocketIndex = pocketIndex, .aimPoint = nativeAimPoint,
...
static double ECAutoplayPowerForPlan(id table, ECAutoplayPlan plan) {
    ...
    // speed for these native friction factors, divide the object's required
...
static BOOL ECAutoplaySetPower(id cue, double power) {
    ...
    Method powerMethod = class_getInstanceMethod([cue class], powerSelector);
    if (!powerMethod) return NO;
    // MCNumber is a non-trivial C++ wrapper. 56.29.2 receives its address in
    // x2 (confirmed at VisualCue::setPower 0x1001d1aec).
    ((void (*)(id, SEL, const double *))method_getImplementation(powerMethod))(
        cue, powerSelector, &power);
    return YES;
}

static BOOL ECAutoplaySetAimPoint(id cue, ECDPoint point) {
    if (!cue || !ECPointValid(point)) return NO;
    SEL pointSelector = NSSelectorFromString(@"pointToTablePosition:");
    Method pointMethod = class_getInstanceMethod([cue class], pointSelector);
    if (!pointMethod) return NO;
    // MCNumberPoint is likewise passed indirectly in x2 (confirmed at
    // VisualCue::pointToTablePosition 0x1001d1fa8).
    ((void (*)(id, SEL, const ECDPoint *))method_getImplementation(pointMethod))(
        cue, pointSelector, &point);
    return YES;
}

static BOOL ECAutoplayReleaseShot(id manager) {
    if (!manager || !ECNativePhysicsSurfaceValid()) return NO;
    uint32_t *releaseCode = ECGameAddress(EC_AUTOPLAY_RELEASE_ADDRESS);
    if (!releaseCode || *releaseCode != EC_AUTOPLAY_RELEASE_OPCODE) {
        ECLogLine(@"autoplay release gate failed");
        return NO;
    }
    void *cacher = ECRawPointerIvar(manager, "_cacher");
    if (!cacher) return NO;
    void *owner = NULL;
    memcpy(&owner, cacher, sizeof(owner));
    if (owner != (__bridge void *)manager) {
        ECLogLine(@"autoplay cacher ownership gate failed");
        return NO;
    }
    ((void (*)(void *))releaseCode)(cacher);
    return YES;
}

// PlayerTurn is state 4. Pass & Play's second human is state 7 and therefore
// reports isOpponentTurn even though the same local cue controls are active.
// Only treat state 7 as ours in a genuinely local game; an online/AI opponent
// must never be driven by this controller.
static BOOL ECAutoplayControlsCurrentTurn(id manager) {
    if (!manager) return NO;
    if (ECInvokeBool(manager, @"isPlayerTurn", NO)) return YES;
    return ECInvokeBool(manager, @"isOnLocalGame", NO) &&
           ECInvokeBool(manager, @"isOpponentTurn", NO);
}

static void ECAutoplayResetRejectedPlans(unsigned int shotId) {
    if (gECAutoplayRejectionShotId == shotId) return;
    memset(gECAutoplayRejectedPockets, 0, sizeof(gECAutoplayRejectedPockets));
    gECAutoplayRejectionShotId = shotId;
}

static BOOL ECAutoplayShadowAcceptsPower(id table, id cueBall, id targetBall,
                                         void *guide, double power,
                                         int expectedPocket, BOOL breakShot,
                                         NSString **reasonOut) {
    if (!table || !cueBall || !targetBall || !guide || !isfinite(power)) {
        if (reasonOut) *reasonOut = @"missing native simulation input";
        return NO;
    }
    Ivar frictionIvar = class_getInstanceVariable([table class], "_frictionProperties");
    const double *cueForceAddress = ECGameAddress(EC_GAME_FORCE_ADDRESS);
    if (!frictionIvar || !cueForceAddress) {
        if (reasonOut) *reasonOut = @"native force/friction unavailable";
        return NO;
    }
    double cueForce = *cueForceAddress;
    if (!isfinite(cueForce) || cueForce <= 0.0) {
        if (reasonOut) *reasonOut = @"native force/friction unavailable";
        return NO;
    }
    const uint8_t *tableBytes = (const uint8_t *)(__bridge const void *)table;
    const double *friction = (const double *)(tableBytes + ivar_getOffset(frictionIvar));
    double initialSpeed = ECShotSpeedForCuePower(power, cueForce);
    EightBPShadowPrediction prediction = {0};
    BOOL succeeded = NO;
    @try {
        succeeded = EightBPShadowPredict(table, cueBall, guide, initialSpeed,
                                         friction, &prediction);
    } @catch (NSException *exception) {
        if (reasonOut) {
            *reasonOut = [NSString stringWithFormat:@"shadow exception %@",
                          exception.name ?: @"unknown"];
        }
        return NO;
    }
    if (!succeeded || !prediction.valid) {
        if (reasonOut) {
            *reasonOut = [NSString stringWithFormat:@"shadow failed: %s",
                prediction.status[0] ? prediction.status : EightBPShadowLastStatus()];
        }
        return NO;
    }

    EightBPShadowBallPrediction *cuePrediction = NULL;
    EightBPShadowBallPrediction *targetPrediction = NULL;
    int movedObjects = 0;
    int pottedObjects = 0;
    BOOL earlyBlack = NO;
    const void *cuePointer = (__bridge const void *)cueBall;
    const void *targetPointer = (__bridge const void *)targetBall;
    for (uint16_t index = 0; index < prediction.ballCount &&
                              index < EightBPShadowMaxBalls; index++) {
        EightBPShadowBallPrediction *ball = &prediction.balls[index];
        if (!ball->valid) continue;
        if (ball->liveBall == cuePointer) cuePrediction = ball;
        if (ball->liveBall == targetPointer) targetPrediction = ball;
        if (ball->number != 0) {
            if (ball->potted) pottedObjects++;
            if (ball->potted || ball->pathPointCount > 1) movedObjects++;
            if (ball->number == 8 && ball->potted &&
                ECBallNumber(targetBall) != 8) earlyBlack = YES;
        }
    }
    if (!cuePrediction || cuePrediction->potted) {
        if (reasonOut) *reasonOut = @"cue ball scratches in native simulation";
        return NO;
    }
    if (earlyBlack) {
        if (reasonOut) *reasonOut = @"8 ball drops before it is legal";
        return NO;
    }
    if (breakShot) {
        // Autoplay is stricter than the bare legal-break rule: do not spend a
        // turn on a spread-only break. Aim variants are tried until the native
        // simulation shows an object ball actually dropping.
        if (pottedObjects == 0) {
            if (reasonOut) *reasonOut = movedObjects < 4
                ? @"break does not move four balls"
                : @"break spreads the rack but pots no ball";
            return NO;
        }
    } else if (!targetPrediction || !targetPrediction->potted) {
        if (reasonOut) *reasonOut = @"target ball does not pot in native simulation";
        return NO;
    }
    if (!breakShot && expectedPocket >= 0 && targetPrediction) {
        ECDPoint pockets[16];
        double pocketRadius = 0.0;
        int pocketCount = ECReadPocketCenters(table, pockets, 16, &pocketRadius);
        if (expectedPocket >= pocketCount || pocketRadius <= 0.0) {
            if (reasonOut) *reasonOut = @"planned pocket is unavailable";
            return NO;
        }
        ECDPoint final = {targetPrediction->finalPosition.x,
                          targetPrediction->finalPosition.y};
        double pocketDistance = hypot(final.x - pockets[expectedPocket].x,
                                      final.y - pockets[expectedPocket].y);
        // The shadow resolver records the ball centre at the native pocket
        // collision. Requiring that event to be at the nominated mouth keeps
        // 8-ball calls exact and prevents accepting an accidental side pot.
        if (!isfinite(pocketDistance) || pocketDistance > pocketRadius * 1.25) {
            if (reasonOut) *reasonOut = @"target pots in a different pocket";
            return NO;
        }
    }
    if (reasonOut) *reasonOut = @"native simulation accepts shot";
    return YES;
}

static BOOL ECAutoplayFindValidatedPower(id table, id cueBall, id targetBall,
                                          void *guide, double requestedPower,
                                          int expectedPocket, BOOL breakShot,
                                          double *powerOut,
                                          NSString **reasonOut) {
    double candidates[7] = {0};
    int count = 0;
    if (breakShot) {
        candidates[count++] = 0.84;
        candidates[count++] = 0.90;
        candidates[count++] = 0.96;
    } else {
        double base = fmin(0.94, fmax(0.34, requestedPower));
        candidates[count++] = base;
        candidates[count++] = fmax(0.34, base - 0.12);
        candidates[count++] = fmin(0.94, base + 0.12);
        candidates[count++] = 0.42;
        candidates[count++] = 0.68;
        candidates[count++] = 0.94;
    }
    NSString *lastReason = @"no power candidate";
    double previous = -1.0;
    for (int index = 0; index < count; index++) {
        double candidate = candidates[index];
        if (fabs(candidate - previous) < 0.0001) continue;
        previous = candidate;
        NSString *reason = nil;
        if (ECAutoplayShadowAcceptsPower(table, cueBall, targetBall, guide,
                                         candidate, expectedPocket, breakShot,
                                         &reason)) {
            if (powerOut) *powerOut = candidate;
...
static BOOL ECAutoplayPlaceCueBall(id manager, id control, NSArray *balls,
    ...
    gECAutoplayBallPlacementFinalized = NO;
    gECAutoplayBallPlacementAt = now;
    gECAutoplayActionAt = now;
    gECAutoplayRetryAfter = now + 0.4;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.18 * NSEC_PER_SEC)),
        ...
        if (ECInvokeBool(liveControl, @"isAnimatingCueBall", NO) &&
            [liveControl respondsToSelector:NSSelectorFromString(@"finishAnimation")]) {
            ((void (*)(id, SEL))objc_msgSend)(liveControl,
                NSSelectorFromString(@"finishAnimation"));
        }
        ...
        gECAutoplayBallPlacementFinalized = YES;
    });
    return YES;
}
...
static void ECAutoplayLegacyTick(void) {
    ...
    ECAutoplayResetRejectedPlans(shotId);
    ...
    if (now - gECAutoplayLastStateLog >= 2.0) {
        ...
        ECLogLine([NSString stringWithFormat:
            @"autoplay state waiting=%d player=%d opponent=%d local=%d controlled=%d moving=%d cue=%d aim=%d touches=%d powerControl=%d ballInHand=%d balls=%lu pending=%d placement=%d shot=%u",
            waiting, playerTurn, opponentTurn, localGame, controlledTurn,
            moving, cueEnabled, aimEnabled, cueHasTouches,
            powerControlEnabled, ballInHand, (unsigned long)balls.count,
            gECAutoplayShotPending, gECAutoplayBallPlacementPending, shotId]);
    }
    ...
    if (ballInHand) {
        ...
        if (controlledTurn) {
            ...
            if (![currentTouches respondsToSelector:@selector(count)] ||
                ...
                if (!gECAutoplayBallPlacementPending) {
                    ECAutoplayPlaceCueBall(manager, ballControl, balls, cueBall, now);
                } else if (gECAutoplayBallPlacementFinalized &&
                           now - gECAutoplayBallPlacementAt > 0.9 &&
                           ECAutoplayCueBallControlActive(manager, ballControl)) {
                    SEL deactivate = NSSelectorFromString(@"deactivateCueBallControl");
                    if ([manager respondsToSelector:deactivate]) {
                        ((void (*)(id, SEL))objc_msgSend)(manager, deactivate);
                        gECAutoplayBallPlacementAt = now;
                        ECLogLine(@"autoplay cue placement finalize retry");
                    }
                }
            }
        }
    ...
    if (gECAutoplayBallPlacementPending) {
        ...
        gECAutoplayBallPlacementAt = 0;
        memset(gECAutoplayRejectedPockets, 0, sizeof(gECAutoplayRejectedPockets));
        gECAutoplayRetryAfter = now + 0.35;
        ECLogLine(@"autoplay cue placement settled");
    }

    if (gECAutoplayShotPending) {
        if (!waiting || shotId != gECAutoplayShotId) gECAutoplaySawShotStart = YES;
        if (gECAutoplaySawShotStart) {
            if (controlledTurn && waiting && !moving && cueEnabled && aimEnabled &&
        ...
        } else if (now - gECAutoplayActionAt > 3.0) {
            ECLogLine(@"autoplay shot did not start; retrying");
            ECAutoplaySetPower(visualCue, 0.0);
            gECAutoplayShotPending = NO;
            gECAutoplayReadySince = 0;
    ...
    // Fail closed until both the game state and the physical table have been
    // continuously ready. This prevents a stale cue/power state from being
    // carried across a turn transition or the tail of a previous shot.
    if (!controlledTurn || !waiting || moving || !cueEnabled || !aimEnabled ||
        !powerControlEnabled || cueHasTouches || now < gECAutoplayRetryAfter) {
        gECAutoplayReadySince = 0;
        return;
    }
    id currentTouches = ECIvarObject(manager, "mCurrentTouches");
    if ([currentTouches respondsToSelector:@selector(count)] && [currentTouches count] != 0) {
        gECAutoplayReadySince = 0;
        return;
    }
    ...
    id englishControl = ECInvokeId(manager, @"visualEnglishControl");
    if (!englishControl) englishControl = ECIvarObject(manager, "mVisualEnglishControl");
    SEL centerEnglish = NSSelectorFromString(@"centerEnglish");
    if ([englishControl respondsToSelector:centerEnglish]) {
        ((void (*)(id, SEL))objc_msgSend)(englishControl, centerEnglish);
    }
    // Aim with an explicitly unloaded cue. Power is applied only after the
    // native guide and shadow simulation have both accepted this exact shot.
    if (!ECAutoplaySetPower(visualCue, 0.0) ||
        !ECAutoplaySetAimPoint(visualCue, plan.aimPoint)) {
        gECAutoplayReadySince = 0;
        gECAutoplayRetryAfter = now + 0.6;
        return;
    }
    if (plan.targetNumber == 8 && plan.pocketIndex >= 0) {
        SEL nominate = NSSelectorFromString(@"nominatePocket:");
        if ([manager respondsToSelector:nominate]) {
            ((BOOL (*)(id, SEL, unsigned int))objc_msgSend)(
                manager, nominate, (unsigned int)plan.pocketIndex);
        }
    }
    ...
    gECAutoplayReadySince = 0;
    ECLogLine([NSString stringWithFormat:
        @"autoplay aim ball=%d pocket=%d direct=%d break=%d scratch=%d requestedPower=%.4f cue=%.2f object=%.2f align=%.3f",
    ...
    BOOL expectedBreak = plan.breakShot;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.35 * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{
        if (!gECAutoplayEnabled || !gECAutoplayShotPending || gECMenuOpen) {
            gECAutoplayShotPending = NO;
            return;
        }
        id liveManager = ECFindGameManager();
        ...
        BOOL noLiveTouches = ![liveTouches respondsToSelector:@selector(count)] ||
            [liveTouches count] == 0;
        BOOL stillReady = liveManager && ECAutoplayControlsCurrentTurn(liveManager) &&
            ECInvokeBool(liveManager, @"isWaitingForPlayerShot", NO) &&
            !ECAutoplayBallsMoving(liveBalls) &&
            ECInvokeBool(liveCue, @"enabled", NO) &&
            ECInvokeBool(liveCue, @"aimEnabled", NO) &&
            !ECInvokeBool(liveCue, @"hasTouches", NO) &&
            noLiveTouches &&
            ECAutoplayCurrentShotId(liveManager) == gECAutoplayShotId;
        if (!stillReady) {
            ECAutoplaySetPower(liveCue, 0.0);
            gECAutoplayShotPending = NO;
            gECAutoplayReadySince = 0;
            gECAutoplayRetryAfter = CACurrentMediaTime() + 0.6;
            ECLogLine(@"autoplay cancelled: table or cue state changed before validation");
            return;
        }
        ...
        if (!actualBall || actualBall != expectedBall) {
            ...
            if (expectedBreak) gECAutoplayBreakVariant++;
            ECLogLine([NSString stringWithFormat:
        ...
        if (!liveCueBall && [liveTable respondsToSelector:@selector(getCueBall)]) {
            liveCueBall = ((id (*)(id, SEL))objc_msgSend)(liveTable, @selector(getCueBall));
        }
        double validatedPower = power;
        NSString *validationReason = nil;
        if (!ECAutoplayFindValidatedPower(liveTable, liveCueBall, expectedBall,
                                           liveGuide, power, expectedPocket,
                                           expectedBreak,
            ...
            if (expectedBreak) {
                gECAutoplayBreakVariant++;
            } else if (expectedNumber > 0 && expectedNumber < 16 &&
                expectedPocket >= 0 && expectedPocket < 8) {
                gECAutoplayRejectedPockets[expectedNumber] |=
                    (uint8_t)(1u << expectedPocket);
            }
            ECLogLine([NSString stringWithFormat:
                @"autoplay rejected by native simulation ball=%d pocket=%d reason=%@",
                expectedNumber, expectedPocket,
                validationReason ?: @"unknown"]);
            return;
        }
        if (!ECAutoplaySetPower(liveCue, validatedPower)) {
            gECAutoplayShotPending = NO;
            gECAutoplayReadySince = 0;
            gECAutoplayRetryAfter = CACurrentMediaTime() + 0.6;
            ECLogLine(@"autoplay cancelled: could not apply validated power");
            return;
        }
        ECLogLine([NSString stringWithFormat:
            @"autoplay native simulation accepted ball=%d pocket=%d power=%.4f",
...
static void ECSyncTableOverlay(void) {
    ...
    static CGPoint lastOrigin = {NAN, NAN};
    static const void *lastParent = NULL;
    static int lastSegmentCount = -1;
    static int lastPocketCount = -1;
    if (!gECShowTableOverlay || !gECInMatch) {
        ECClearTableOverlay();
        lastScale = 0;
        lastOrigin = CGPointMake(NAN, NAN);
        lastParent = NULL;
        lastSegmentCount = -1;
        lastPocketCount = -1;
        return;
    ...
    if (!ECLooksLikeObject(reference)) {
        for (int slot = 0; slot < gECCachedSnapCount; slot++) {
            if (ECLooksLikeObject(gECCachedBalls[slot])) {
                reference = gECCachedBalls[slot];
                break;
            }
        }
    }
    ...
    id parent = ECLooksLikeObject(sphere) ? ECInvokeId(sphere, @"parent") : nil;
    if (!ECLooksLikeObject(table) || !ECLooksLikeObject(reference) ||
        !ECLooksLikeObject(parent)) return;

    CGPoint origin = CGPointMake(NAN, NAN);
    if (!ECReadPointIvar(reference, "mVisualTableOrigin", &origin)) return;
    double scale = gECDerivedVisualScale > 0.2 ? gECDerivedVisualScale : 1.7007874015748;

    ECDPoint shapePoints[128];
    int shapeCount = 0;
    Ivar shapeIvar = class_getInstanceVariable([table class], "mTableShape");
    if (shapeIvar) {
        const uint8_t *tableBytes = (const uint8_t *)(__bridge const void *)table;
        const ECPointVector *shape =
            (const ECPointVector *)(tableBytes + ivar_getOffset(shapeIvar));
        if (shape && shape->begin && shape->end && shape->end > shape->begin) {
            ptrdiff_t count = shape->end - shape->begin;
            if (count >= 4 && count <= 128) {
                for (ptrdiff_t index = 0; index < count; index++) {
                    if (ECPointValid(shape->begin[index])) {
                        shapePoints[shapeCount++] = shape->begin[index];
                    }
                }
            }
        }
    ...
    ECDPoint pockets[16];
    double pocketRadius = 6.7;
    int pocketCount = ECReadPocketCenters(table, pockets, 16, &pocketRadius);
    if (pocketCount <= 0) {
        ECDBox box = ECDefaultTableBox();
        pockets[0] = (ECDPoint){box.minX, box.minY};
        pockets[1] = (ECDPoint){box.minX, box.maxY};
        pockets[2] = (ECDPoint){box.maxX, box.minY};
        pockets[3] = (ECDPoint){box.maxX, box.maxY};
        pockets[4] = (ECDPoint){0.0, box.minY};
        pockets[5] = (ECDPoint){0.0, box.maxY};
        pocketCount = 6;
    }

    if (gECTableOverlaySprites.count > 0 &&
        fabs(scale - lastScale) < 1e-6 &&
        fabs(origin.x - lastOrigin.x) < 0.05 &&
        fabs(origin.y - lastOrigin.y) < 0.05 &&
        lastParent == (__bridge const void *)parent &&
        lastSegmentCount == shapeCount &&
    ...
    if (shapeCount >= 2) {
        ...
        int edges = closeGap < 12.0 ? shapeCount : shapeCount - 1;
...
- (void)closePanel {
    ...
    // EmberMenuPanel owns UIKit controls whose associated action blocks are
    // rebuilt on every render.  Keeping the whole tree hidden across 8 Ball's
    // window/scene transitions left those controls attached to a stale guest
    // window and caused the next open to abort inside UIKit.  Match the stable
    // shared-menu lifecycle used by the other tweaks: discard and recreate it.
...
static void EmberEightBPOfflineLinesBoot(void) {
    ...
    gECAutoplayEnabled = [defaults objectForKey:ECAutoplayKey] == nil
        ? NO : [defaults boolForKey:ECAutoplayKey];
